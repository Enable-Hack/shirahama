...
## ⚠ 致命的: タイムゾーン (TZ) — 全 skill が必ず踏まえる

- **ユーザ入力は常に JST** (受電 form / `/incident <時間窓>` 引数 / hearing JSON / すべて)
- **サーバ側は UTC の可能性が極めて高い** (cloud image のデフォルト、OCI Rocky 確認済 = UTC、本番 victor / bravo は未確認だが要注意)
- `/incident` は ssh で `date +%z` を叩いて各サーバ TZ を **自動検出** → 入力 JST を各サーバ TZ に変換してから awk filter する (修正済 2026-05-05)
- 事故シナリオ: 「12:00 JST」と入力 → サーバ UTC → 本来は UTC 03:00 のログを見るべきだが、修正前は 9 時間ズレた UTC 12:00 (= JST 21:00) のログを取ってた
- skill 起動時の出力に「victor TZ: +0000 → ローカル時刻 03:00〜03:30」のような変換結果が表示される。これが受電内容と整合してないと致命的事故 → 必ず確認
