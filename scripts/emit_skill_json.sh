...
#   1) --incident-id 明示指定
#   2) 環境変数 INCIDENT_ID
#   3) 自動生成 ${TODAY_UTC}_${WINDOW_START}_${HOST}
#   4) fallback ${TODAY_UTC}T${HMS}_unscoped
...
if [ -n "$INCIDENT_ID_ARG" ]; then
    ...
    RESOLVED_ID="$(date -u +%Y-%m-%d)_${WINDOW_START}_${HOST:-unknown}"
else
    RESOLVED_ID="$(date -u +%Y-%m-%dT%H%M%S)_unscoped"
fi
